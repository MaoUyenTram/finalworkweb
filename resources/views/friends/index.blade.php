<x-app-layout>
    ...
    @if(Session::has('succes'))
        <div class="bg-green-100 border-t border-b border-green-500 text-green-700 px-4 py-3 text-center" role="alert">{{ Session::get('succes') }}</div>
    @endif
    @if($errors->any())
                ...
                <div class="float-left">
                    <form class="w-full max-w-sm" method="post" action="{{route('friends-and-bans.store')}}">
                        ...
                        <div class="flex items-center border-b border-teal-500 py-2">
                            <input class="appearance-none bg-transparent border-none w-full text-gray-700 mr-3 py-1 px-2 leading-tight focus:outline-none" name="name" type="text" placeholder="JaneDoe#1" aria-label="name">
                    ...
                    <table class="w-full text-right table-auto">
                        ...
                        @foreach($friends as $friend)
                            <tr class=" px-4 py-2">
                                <td>{{$friend->name}}</td>
                                <td>
                                    <form class="w-full max-w-sm" method="post" action="{{route('friends-and-bans.update',$friend->name)}}">
                                ...
                                <td>
                                    <form class="w-full max-w-sm" method="post" action="{{route('friends-and-bans.destroy',$friend->name)}}">
                                        <input type="hidden" name="name" value={{$friend->name}}>
                                        @method('DELETE')
                                        @csrf
                    ...
                    <table class="w-full text-right table-auto">
                        ...
                        @foreach($bans as $ban)
                            <tr class=" px-4 py-2">
                                <td>{{$ban->name}}</td>
                                <td>
                                    <form class="w-full max-w-sm" method="post" action="{{route('friends-and-bans.update',$ban->name)}}">
                                ...
                                <td>
                                    <form class="w-full max-w-sm" method="post" action="{{route('friends-and-bans.destroy',$ban->name)}}">
                                        <input type="hidden" name="name" value={{$ban->name}}>
                                        @method('DELETE')
                                        @csrf
                                        <button class="flex-shrink-0 bg-red-500 hover:bg-teal-700 border-teal-500 hover:border-teal-700 text-sm border-4 text-white py-1 px-2 rounded" type="submit">remove</button>
